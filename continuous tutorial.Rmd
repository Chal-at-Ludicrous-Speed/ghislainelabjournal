...
mydata_CT <- sienaDataCreate(friendship, selfesteem, fem)
```
...
```{r}
myeff_CT <- includeEffects(myeff_CT, transTrip, transRecTrip, inPop, inAct, outAct)
```
...
```{r, eval=TRUE}
myeff_CT <- includeEffects(myeff_CT, egoX, altX, sameX, interaction1 = "fem")
```

Third, we model the effect of self-esteem on the friendship network dynamics.
```{r}
myeff_CT <- includeEffects(myeff_CT, egoX, altX, egoXaltX, interaction1 = "selfesteem")
```
...
```{r}
myeff_CT <- includeEffects(myeff_CT, name = "selfesteem", indeg, interaction1 = "friendship")
```
...
```{r}
res <- siena07(myalg(3000, firstg = 0.1), data = mydata_CT, effects = myeff_CT,
...
ansSim <- siena07(simAlgorithm, data = mydata_CT, effects = myeff_CT, 
