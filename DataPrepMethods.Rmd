...
To analyse the effect of prestige on the network of Dutch sociologists, I will create a sample representative of the population.

The dataset consists of directed ties in a network of scholars working at a Dutch sociology department and scholars working at a Dutch political science department. By using the OpenAlex API, the IDs of the works affiliated with these scholars had been webscraped. In addition to the existing data, I personally webscraped the h-index of OpenAlex. This is publicly available data. The sample includes sociologists who work or have worked at sociology departments in the Netherlands and have an OpenAlex profile. This profile is necessary, as it gives access to much information such as amount of publications, work collaborations for those publications and h-index.
...
R will be used to calculate Stochastic Actor Oriented Models (SAOM). These are random network configurations, where each estimate generates a new, random model.

First, a descriptive model will be run to answer the first research question. This model will show the distribution of scholars, their respective departments and their level of prestige across the network of all Dutch sociologists. 
Secondly, several explanatory models will be run using the RSiena package. 
The node is an author, and a tie is the article written by  authors. The network is modelled as a directed network. Therefore, the first author of the paper is considered to be the receiver of the tie by the other authors of the same paper. 

The first model is a null model, where two variables are included. Then two separate models are run where one independent variable is included to answer the separate hypotheses, followed by a model where both independent variables are included. Finally, a model with the independent and control variables is run.     

## Measures 

To operationalise the concept of ‘academic prestige’, the personal h-index was used as a proxy, representing individual prestige. 

Scholars with more career seniority and a permanent position are more likely to be representative of the level of quality of an academic department than those who are more junior. The functions in the data are also somewhat ambiguous, with some being listed in more general terms like lecturer or staff, and others more specifically like full professor. Being a ‘Lecturer’ differs per university and per academic, as they can be academics who have only completed their MA, are currently working on their PhD promotion or have already obtained their PhD. Therefore, sociologists who have a function as assistant professor, associate professor or full professor were taken into the departmental prestige measure. The h-index of these members were counted up to calculate the mean h-index. This became the departmental prestige measure. 
...
While many changes can happen to someone’s academic function, such as being hired, leaving or getting a promotion, most remain stable. For the sake of the analysis, the functions were standardized. 
...
Besides the main effects of individual and departmental prestige, two control variables were also taken into the analysis. The first is a numeric version of the university variable and the second is a dummy variable of gender. Both are used to control for the effect that scholars may prefer working together with others from their own department, or who share the same gender [@McPherson]. This is achieved through the sameX effect, which examines whether both the sender and the receiver of a tie share a certain characteristic.

After removing the 2025 time point, some missingness remained in university, function and gender. This missingness was very small. Removing the scholars with missing variables proved to be more problematic to run the RSiena model than keeping them in the dataset.
